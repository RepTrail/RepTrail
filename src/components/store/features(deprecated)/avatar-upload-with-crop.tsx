        ...
        <div className={`flex flex-col ${alignmentClasses[align]} gap-4 w-full`}>
            <div className="relative group">
                ...
                <div
                    ...
                    className={`w-36 h-36 rounded-system bg-zinc-800 border-4 border-zinc-700/50 flex items-center justify-center overflow-hidden shadow-2xl group cursor-pointer transition-all duration-500 relative ${colorClasses}`}
                >
                    {uploading && (
                ...
                <Button
                    ...
                    size="icon"
                    onClick={() => fileInputRef.current?.click()}
                    className="absolute -bottom-2 -right-2 h-10 w-10 rounded-system bg-zinc-900 border-zinc-800 shadow-xl hover:bg-zinc-800 transition-all z-20"
                >
            ...
            <Dialog open={isDialogOpen} onOpenChange={(open) => {
                ...
                <DialogContent className="sm:max-w-[500px] bg-zinc-950 border-zinc-800 p-0 overflow-hidden rounded-system">
                    ...
                    <div className="p-6 space-y-6">
                        <div className="space-y-3">
                            ...
                            <input
                                ...
                                className={`w-full h-1.5 bg-zinc-800 rounded-system appearance-none cursor-pointer accent-orange-500 ${accentColor === 'emerald' ? 'accent-emerald-500' : 'accent-orange-500'}`}
                            />
                        </div>
                        ...
                        <DialogFooter className="flex gap-3 sm:justify-end">
                            <Button
                                ...
                                disabled={uploading}
                                className="border-zinc-800 bg-zinc-900 text-zinc-400 hover:text-white rounded-system h-12 uppercase font-bold text-xs"
                            >
                                Cancelar
                            </Button>
                            <Button
                                ...
                                disabled={uploading}
                                className={`rounded-system h-12 px-8 uppercase font-black italic tracking-wide active:scale-95 transition-all flex items-center gap-2 ${btnClasses}`}
                            >
