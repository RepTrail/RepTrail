        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            className="bg-zinc-900/50 border-zinc-800 text-sm h-12 rounded-system text-white"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className={cn(
                            "flex items-center bg-zinc-900/50 border border-zinc-800 rounded-system h-14 pr-2 transition-all overflow-hidden",
                            currentTheme.focus
                        )}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {DAYS.map((day) => (
                            <Button
                                ...
                                className={cn(
                                    "h-11 text-[10px] font-black uppercase tracking-widest rounded-system transition-all active:scale-95 border-2",
                    ...
                    {formData.application_days.length > 0 && (
                        <div className={cn("p-4 rounded-system flex items-center justify-between border", currentTheme.infoBg)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    ...
                    <Input type="date" value={formData.start_date} onChange={e => setFormData({ ...formData, start_date: e.target.value })} className="bg-zinc-900/50 border-zinc-800 text-sm h-12 rounded-system text-white" />
                </div>
                <div className="space-y-2">
                    <Label className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest ml-1">Previsão Término</Label>
                    <Input type="date" value={formData.end_date} onChange={e => setFormData({ ...formData, end_date: e.target.value })} className="bg-zinc-900/50 border-zinc-800 text-sm h-12 rounded-system text-white" />
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <Textarea
                    ...
                    className="bg-zinc-900/50 border-zinc-800 text-sm rounded-system min-h-[100px] text-white"
                />
            </div>

            <div className="flex justify-end gap-3 pt-6 border-t border-zinc-900">
                <Button variant="ghost" onClick={onCancel} className="text-zinc-500 hover:text-white rounded-system h-11 px-6 font-bold uppercase tracking-widest text-[10px]">Cancelar</Button>
                 <Button 
                    onClick={() => onSubmit(formData)} 
                    disabled={!formData.name || formData.weekly_dosage <= 0 || formData.application_days.length === 0} 
                    className={cn(currentTheme.button, "text-white rounded-system h-11 px-8 font-bold shadow-lg transition-all active:scale-95")}
                >
                    {submitLabel}
                </Button>
            </div>
