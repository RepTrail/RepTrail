...
import { PdfUploader } from '@/components/store/features(deprecated)/pdf-uploader'
...
export function ImportPdfClient({ students, role = 'trainer', userId }: { students?: any[], role?: 'trainer' | 'student', userId: string }) {
    ...
    if (!isMounted) {
        return <div className="animate-pulse bg-zinc-900 h-96 rounded-system" />
    }
    ...
    return (
        <div className="space-y-10 pb-10" suppressHydrationWarning>
            <Tabs defaultValue="workout" className="w-full">
                <TabsList
                    id="tour-import-type"
                    className="bg-zinc-950 border border-zinc-800 p-1 h-14 rounded-system w-full max-w-md shadow-2xl"
                >
                    <TabsTrigger
                        value="workout"
                        className="rounded-system group data-[state=active]:bg-zinc-900 data-[state=active]:border-zinc-700 border border-transparent transition-all h-full"
                    >
