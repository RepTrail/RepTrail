    ...
    const renderErgogenicCard = (e: Ergogenic, isToday: boolean, idx: number) => {
        ...
        return (
            <Card key={e.id || `ergo-${idx}`} className={cn(
                "bg-zinc-900/50 border-zinc-800 text-zinc-100 transition-all group rounded-system overflow-hidden flex flex-col h-full",
                colors.border,
                isToday && isDone && colors.activeBg
            )}>
                <CardHeader className="p-6 pb-4">
                    <div className="flex items-start justify-between">
                        <div className={cn("bg-zinc-800 p-2 rounded-system transition-colors", colors.icon)}>
                        ...
                        {mode === 'trainer' && (
                            <div className="flex items-center gap-2">
                                <Button
                                    ...
                                    className={cn("h-8 w-8 text-zinc-500 rounded-system transition-colors", colorScheme === 'orange' ? 'hover:text-orange-500' : 'hover:text-emerald-500')}
                                >
                ...
                <CardContent className="p-6 pt-0 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-6">
                        {WEEKDAYS_SHORT.map((day, idx) => {
                            ...
                            return (
                                <span key={idx} className={cn("flex items-center shrink-0 gap-1 px-2 py-1 text-[9px] font-black uppercase rounded-system border", colors.badge)}>
                    ...
                    {e.notes && (
                        <div className="mb-6 p-4 bg-zinc-950/50 rounded-system border border-zinc-800/50">
                    ...
                    <div className="mt-auto pt-6 border-t border-zinc-800/50 flex items-center gap-2">
                        {mode === 'student' && isToday ? (
                            <Button
                                ...
                                className={cn(
                                    "flex-1 h-9 rounded-system text-[10px] font-black uppercase tracking-widest transition-all gap-1.5 shadow-lg",
                            ...
                            <>
                                <UnifiedAssignDialog
                                    ...
                                    trigger={
                                        <Button
                                            className={cn("flex-1 min-w-0 h-9 rounded-system text-[10px] font-black uppercase tracking-widest transition-all shadow-lg flex items-center justify-center gap-1.5 px-3", colors.button)}
                                        >
                                ...
                                <UnifiedCreationDialog
                                    ...
                                    trigger={
                                        <Button variant="outline" className="flex-1 min-w-0 h-9 bg-zinc-800 border-zinc-700 text-zinc-100 hover:bg-zinc-700 flex items-center justify-center gap-1.5 rounded-system font-black text-[10px] uppercase italic tracking-widest border-white/5 px-3">
                                            Editar
                                        </Button>
                                    }
                                />
        ...
        <div className="space-y-12 animate-in fade-in duration-700">
            ...
            <div className="space-y-8">
                ...
                {todaysErgogenics.length === 0 && otherErgogenics.length === 0 && (
                    <div className="py-20 text-center border-dashed border border-zinc-800 rounded-system w-full">
            ...
            {mode === 'student' && logs.length > 0 && (
                <div className="space-y-6">
                    ...
                    <Card className="bg-zinc-900/40 border-zinc-800/50 rounded-system overflow-hidden shadow-2xl backdrop-blur-sm">
                                ...
                                {logs.map((log: any, index: number) => (
                                    <div key={log.id || `log-${index}`} className="p-6 flex items-center justify-between hover:bg-zinc-800/30 transition-all group">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className={cn("p-3 bg-zinc-950 rounded-system border border-zinc-800 transition-all", colorScheme === 'orange' ? 'group-hover:bg-orange-500/10 group-hover:border-orange-500/20' : 'group-hover:bg-emerald-500/10 group-hover:border-emerald-500/20')}>
