...
import { WorkoutPreviewDialog } from './student-workout-preview-dialog'
        ...
        <Stack gap={STORE_TOKENS.SPACING.EMPTY_STATE}>
            {allowManualWorkouts && (
                <div className="flex items-center justify-end gap-3 w-full">
                    <Button asChild variant="outline" className="h-12 px-6 bg-zinc-900 border-zinc-700 hover:bg-zinc-800 hover:text-white rounded-system text-[10px] font-black uppercase tracking-widest text-zinc-300 transition-all flex items-center justify-center gap-2">
                ...
                workouts.length === 0 ? (
                    ...
                    <Grid gap={STORE_TOKENS.SPACING.CONTAINER} mdCols={2} lgCols={3}>
                        {workouts.map((assignment: any) => {
                            ...
                            return (
                                <Card key={assignment.id} className="bg-zinc-900/40 border-zinc-800/50 hover:border-orange-500/30 transition-all duration-300 shadow-2xl rounded-system overflow-hidden group backdrop-blur-sm">
                                    <CardHeader className="p-8 pb-4 space-y-6">
                                        <div className="flex justify-between items-start">
                                            <div className="p-4 bg-zinc-950 rounded-system border border-zinc-800 group-hover:bg-orange-500/10 group-hover:border-orange-500/20 shadow-2xl transition-all duration-300">
                                                <Dumbbell className="w-8 h-8 text-zinc-700 group-hover:text-orange-500 transition-colors shadow-inner" />
                                            </div>
                                            <div className="bg-zinc-950 border border-zinc-800 px-3 py-2 rounded-system flex items-center gap-1.5 shadow-2xl">
                    ...
                    <Grid gap={STORE_TOKENS.SPACING.CONTAINER} mdCols={2} lgCols={3}>
                        {libraryWorkouts.length > 0 ? (
                            libraryWorkouts.map((workout: any) => {
                                ...
                                return (
                                    <Card key={workout.id} className="bg-zinc-900/50 border-zinc-800 text-zinc-100 hover:border-orange-500/30 transition-all group rounded-system overflow-hidden flex flex-col">
                                        ...
                                        <CardContent>
                                            ...
                                            <div className="mt-auto pt-6 border-t border-zinc-800/50 flex items-center gap-2">
                                                <UnifiedAssignDialog
                                                    ...
                                                    trigger={
                                                        <Button
                                                            className="flex-1 min-w-0 h-9 bg-orange-500 hover:bg-orange-400 text-zinc-950 rounded-system text-[10px] font-black uppercase tracking-widest transition-all shadow-lg flex items-center justify-center gap-1.5 px-6"
                                                        >
                                                    ...
                                                    queryKey={QUERY_KEYS.workouts.all(userId)}
                                                />
                                                <Button asChild variant="outline" className="flex-1 min-w-0 h-9 bg-zinc-800 border-zinc-700 text-zinc-100 hover:bg-zinc-700 flex items-center justify-center gap-1.5 rounded-system font-black text-[10px] uppercase italic tracking-widest border-white/5 px-6">
