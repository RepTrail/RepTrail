...
export function UnifiedExerciseChart({
    ...
    if (data.length === 0 || processedData.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-4 bg-zinc-950/30 rounded-system border border-dashed border-zinc-800/50">
        ...
        return (
            <div className={cn("grid gap-6 md:grid-cols-3", className)}>
                {(processedData as any[]).map((item) => (
                    <Card key={item.name} className="bg-zinc-900/40 border-zinc-800/80 rounded-system overflow-hidden group hover:border-emerald-500/50 transition-all shadow-2xl backdrop-blur-sm">
                        ...
                        <CardContent className="p-8 pt-0">
                            ...
                            <div className="mt-6 flex items-center justify-between">
                                ...
                                <div className="w-10 h-10 bg-emerald-500/10 rounded-system flex items-center justify-center border border-emerald-500/20 group-hover:bg-emerald-500 group-hover:text-zinc-950 transition-all duration-500">
    ...
    return (
        <div className={cn("space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700", className)}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                ...
                {weightEvo !== 0 && (
                    <div className={cn(
                        "flex items-center gap-4 px-6 py-3 rounded-system border-2 self-start",
            ...
            <div className="bg-zinc-950/50 border border-zinc-800/50 rounded-system p-8 space-y-6 shadow-3xl backdrop-blur-xl">
                <div
                            ...
                            return (
                                <div key={i} className="flex flex-col items-center gap-4 group">
                                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-y-2 group-hover:translate-y-0">
                                        <div className="bg-emerald-500 text-zinc-950 text-[10px] font-black px-2 py-1 rounded-system shadow-lg">
                                            {p.val}kg
                                        </div>
                                    </div>
                                    <div className="relative flex flex-col items-center justify-end h-32 w-14">
                                        <div
                                            className="w-full bg-gradient-to-t from-emerald-600 to-emerald-400 rounded-system transition-all duration-500 group-hover:brightness-125 group-hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]"
            ...
            <div className="bg-zinc-900/40 p-6 rounded-system border border-zinc-800/50 flex flex-col sm:flex-row gap-4 sm:items-center">
                <div className="w-12 h-12 rounded-system bg-zinc-950 flex items-center justify-center border border-zinc-800 shadow-inner">
