...
export function PodiumCard({ trainer, rank }: { trainer: TrainerRanking, rank: number }) {
    ...
    return (
        <Card className={`group relative bg-zinc-900 border-zinc-800 shadow-2xl rounded-system overflow-hidden transition-all duration-700 hover:border-orange-500/40 h-full`}>
            ...
            <CardContent className="p-6 sm:p-10 flex flex-col items-center text-center space-y-8 relative z-10">
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-center gap-2">
                        <div className="flex items-center gap-1.5 px-3 py-0.5 bg-orange-500/5 border border-orange-500/20 rounded-system">
                ...
                {trainer.trainer_code ? (
                    <Link href={`/personal/${trainer.trainer_code.toUpperCase().trim()}`} className="w-full pt-4">
                        <Button className="w-full h-auto min-h-[3.5rem] py-4 rounded-system bg-white hover:bg-orange-500 hover:text-zinc-950 text-zinc-950 font-black uppercase italic tracking-wide transition-all shadow-xl active:scale-95">
                            Ver Perfil
                    ...
                    <div className="w-full pt-4">
                        <Button disabled className="w-full h-auto min-h-[3.5rem] py-4 rounded-system bg-zinc-800 text-zinc-600 font-black uppercase italic tracking-wide cursor-not-allowed">
                            Sem código
                        </Button>
                    </div>
                )}
            ...
            <div className="hidden md:flex items-center gap-12 ml-6">
                ...
                {trainer.trainer_code ? (
                    <Link href={`/personal/${trainer.trainer_code.toUpperCase().trim()}`}>
                        <Button variant="outline" className="h-auto min-h-[3rem] py-3 px-6 rounded-system border-zinc-800 bg-transparent hover:bg-white hover:text-zinc-950 text-white font-black uppercase italic tracking-wide transition-all group/btn">
                            Ver Perfil
                            <ArrowRight className="w-4 h-4 ml-2 group-hover/btn:translate-x-1 transition-transform" />
                        </Button>
                    </Link>
                ) : (
                    <Button disabled variant="outline" className="h-12 px-6 rounded-system border-zinc-800 bg-transparent text-zinc-700 font-black uppercase italic tracking-wide cursor-not-allowed">
                        Sem código
                    </Button>
                )}
            </div>
