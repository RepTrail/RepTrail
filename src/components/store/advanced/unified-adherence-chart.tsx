...
export function UnifiedAdherenceChart({ history, showErgogenics = false, noCard = false }: AdherenceChartProps) {
    ...
    const rows = React.useMemo(() => [
        ...
        { id: 'diet', label: 'Dieta', icon: Utensils, color: '#3b82f6' }, // Blue not in primary tokens
    ...
    const getStatusStyles = (status: string, percentage?: number) => {
        if (percentage !== undefined && percentage !== null && status !== 'none' && status !== 'assigned') {
            ...
            if (status === 'skipped' || status === 'fail') return { bg: STORE_TOKENS.COLORS.DANGER, shadow: 'none', border: 'none' }
            return { bg: STORE_TOKENS.COLORS.BACKGROUND_ELEVATED, shadow: 'none', border: 'none' }
        }
        switch (status) {
            ...
            case 'skipped': return { bg: STORE_TOKENS.COLORS.DANGER, shadow: 'none', border: 'none' }
            case 'assigned': return { bg: STORE_TOKENS.COLORS.BACKGROUND_ELEVATED, shadow: 'none', border: \1px solid \\ }
            case 'none': default: return { bg: STORE_TOKENS.COLORS.BACKGROUND, shadow: 'none', border: \1px solid \\, opacity: 0.4 }
        }
    }

    const content = (
        <Stack fullWidth gap={STORE_TOKENS.SPACING.SECTION} paddingBottom={noCard ? 0 : 4}>
            <Stack fullWidth gap={STORE_TOKENS.SPACING.ELEMENT} style={{ minWidth: '450px' }}>
                {rows.map(row => (
                    <Grid key={row.id} cols={{ base: '90px 1fr' }} gap={STORE_TOKENS.SPACING.CONTAINER} align="center">
                        ...
                        <Stack direction="row" align="center" gap={STORE_TOKENS.SPACING.TINY} bg="zinc" bgOpacity={30} padding={1.5} rounded={STORE_TOKENS.RADIUS.SYSTEM} border={true} borderColor="zinc" borderOpacity={10} style={{ height: '28px' }}>
                                            ...
                                            <TooltipContent side="top" className="bg-zinc-900 border-zinc-800 text-white text-[10px] p-3 rounded-system shadow-2xl backdrop-blur-xl">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="font-bold text-white italic uppercase">
                                                        {row.id === 'diet'
                                                            ? \\% de adesão\
                                                            : status === 'none' ? 'Folga / Sem Meta' : status === 'assigned' ? 'Pendente' : status === 'skipped' ? 'Falhou' : status === 'partial' ? \Parcial (\%)\ : 'Concluído'}
                                                    </p>
                                                </div>
                                            </TooltipContent>
                                        </Tooltip>
            ...
            <Stack direction="row" wrap="wrap" align="center" justify="center" gap={STORE_TOKENS.SPACING.SECTION} style={{ opacity: 0.6, marginTop: '2rem' }}>
                ...
                <LegendItem color={STORE_TOKENS.COLORS.DANGER} label="Não Realizado" />
                <LegendItem color={STORE_TOKENS.COLORS.BACKGROUND_ELEVATED} label="Programado" />
            </Stack>
        </Stack>
    )
