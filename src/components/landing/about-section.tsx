...
export function AboutSection() {
    return (
        <section className="py-[var(--spacing-app-section)] px-[var(--spacing-app-container)] bg-surface-950 border-b border-zinc-900 w-full">
            <div className="w-full max-w-[1100px] mx-auto flex flex-col gap-[var(--spacing-app-item)] md:gap-[var(--spacing-app-section)]">
                <div className="flex flex-col items-center gap-[var(--spacing-app-item)] text-center max-w-2xl mx-auto">
                    <h2 className="text-3xl md:text-5xl font-black text-white uppercase italic tracking-tighter">
                        Infraestrutura <span className="text-brand-primary">Professional.</span>
                    </h2>
                ...
                <div className="flex flex-col md:flex-row flex-wrap gap-[var(--spacing-app-item)] w-full justify-between">
                    {features.map((feature, idx) => (
                        <div
                            key={idx}
                            className="flex flex-col w-full md:w-[calc(50%-10px)] group p-8 rounded-[2rem] bg-zinc-900/40 border border-zinc-800 hover:border-brand-primary/30 hover:bg-zinc-900/60 transition-all duration-300 relative overflow-hidden"
                        >
                            ...
                            <div className="w-14 h-14 rounded-2xl bg-zinc-950 flex items-center justify-center border border-zinc-800 mb-6 group-hover:border-brand-primary/50 group-hover:text-brand-primary transition-colors shadow-lg shrink-0 z-10">
                                <feature.icon className="w-7 h-7 text-white group-hover:text-brand-primary transition-colors" />
                            </div>

                            <h3 className="text-xl font-black text-white uppercase italic mb-3 tracking-wide group-hover:text-brand-primary transition-colors z-10">
                                {feature.title}
                            </h3>
                            ...
                            <ul className="flex flex-col gap-[10px] pt-4 border-t border-zinc-800/50 z-10">
                                {feature.bullets.map((bullet, i) => (
                                    <li key={i} className="flex items-center gap-[10px] text-xs font-bold text-zinc-500 uppercase tracking-wide group-hover:text-brand-primary/80 transition-colors">
