...
export function StudentFeatures() {
    return (
        <section className="py-[var(--spacing-app-section)] px-[var(--spacing-app-container)] bg-surface-950 w-full relative overflow-hidden flex flex-col items-center border-b border-zinc-900">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-brand-secondary/10 via-surface-950 to-surface-950 opacity-40" />
            ...
            <div className="w-full max-w-[1100px] mx-auto flex flex-col lg:flex-row gap-[var(--spacing-app-section)] items-center justify-between relative z-10">
                ...
                <div className="relative group flex justify-center lg:justify-start w-full lg:w-[calc(50%-25px)] animate-in fade-in slide-in-from-left-8 duration-1000">
                    <div className="absolute inset-0 bg-brand-secondary blur-[80px] opacity-20 group-hover:opacity-30 transition-opacity duration-1000 w-[80%] mx-auto lg:mx-0" />
                ...
                <div className="flex flex-col gap-[var(--spacing-app-item)] md:gap-[40px] text-left w-full lg:w-[calc(50%-25px)] animate-in fade-in slide-in-from-right-8 duration-1000 delay-200">
                    <div className="flex flex-col gap-[var(--spacing-app-item)] items-start text-left">
                        <div className="inline-flex items-center rounded-full border border-brand-secondary/20 bg-brand-secondary/5 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-brand-secondary">
                        ...
                        <h2 className="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter leading-tight">
                            ...
                            <span className="text-brand-secondary">Maestria.</span>
                        </h2>
                    ...
                    <div className="flex flex-col sm:flex-row flex-wrap gap-[var(--spacing-app-item)] text-left">
                        {features.map((item, idx) => (
                            <div key={idx} className="flex flex-1 min-w-[200px] items-center gap-[var(--spacing-app-item)] group p-3 rounded-xl hover:bg-zinc-900/50 transition-colors border border-transparent hover:border-zinc-800">
                                <div className="w-8 h-8 rounded-lg bg-brand-secondary/10 flex items-center justify-center border border-brand-secondary/20 group-hover:border-brand-secondary/50 transition-colors shrink-0">
                                    <item.icon className="w-4 h-4 text-brand-secondary" />
                                </div>
                                <span className="text-sm font-bold text-zinc-300 uppercase italic tracking-wide group-hover:text-brand-secondary transition-colors">
                                    {item.text}
                                </span>
                            </div>
                        ))}
