...
import { Check, Flame, Dumbbell, Utensils, Activity, ArrowLeft } from "lucide-react";
import Link from "next/link";
import { ArrowRight } from "lucide-react";

const features = [
    "App intuitivo Mobile-First",
    "Histórico de Cargas & Progressão",
    "Protocolo de Dieta e Ergogênicos",
    "Ranking Global Gamificado",
    "Controle de Macros e Calorias",
    "Fotos Comparativas (Antes/Depois)",
];

export function StudentFeatures() {
    return (
        <section className="py-24 bg-zinc-950 px-4 md:px-6 relative overflow-hidden flex flex-col items-center">
            ...
            <div className="container mx-auto grid lg:grid-cols-2 gap-16 items-center relative z-10">
                ...
                <div className="order-2 lg:order-1 relative group flex justify-center lg:justify-end animate-in fade-in slide-in-from-left-8 duration-1000">
                    ...
                    <div className="relative w-[300px] h-[600px] bg-zinc-950 border-[8px] border-zinc-900 rounded-[3rem] shadow-2xl overflow-hidden ring-1 ring-zinc-800">
                        {/* Mock Header */}
                        ...
                        <div className="pt-28 px-6 pb-8 h-full bg-zinc-950 flex flex-col gap-6 overflow-hidden">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-xl font-black text-white italic uppercase">Treino A</h3>
                                    <span className="text-[10px] font-bold text-orange-500 uppercase tracking-widest bg-orange-500/10 px-2 py-1 rounded">Em andamento</span>
                                </div>
                                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Peito & Tríceps • Foco Força</p>
                            </div>
                            ...
                            <div className="bg-zinc-900/60 p-4 rounded-3xl border border-zinc-800 space-y-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-white text-sm">Supino Reto</h4>
                                        <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">4 Séries • 8-12 Reps</p>
                                    </div>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="bg-zinc-950 p-2 rounded-xl text-center border border-zinc-800">
                                        <span className="text-[8px] text-zinc-600 block uppercase font-bold">Carga</span>
                                        <span className="text-sm font-black text-white italic">80kg</span>
                                    </div>
                                    <div className="bg-zinc-950 p-2 rounded-xl text-center border border-zinc-800">
                                        <span className="text-[8px] text-zinc-600 block uppercase font-bold">Reps</span>
                            ...
                            {/* Diet Snap */}
                            <div className="bg-zinc-900/60 p-4 rounded-3xl border border-zinc-800 flex items-center gap-4 opacity-60 scale-95">
                                <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500">
                                    <Utensils className="w-5 h-5" />
                                </div>
                                <div>
                                    <h4 className="font-bold text-zinc-300 text-xs uppercase">Almoço</h4>
                                    <p className="text-[8px] text-zinc-600 font-bold uppercase tracking-widest">150g Frango • 100g Arroz</p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="order-1 lg:order-2 space-y-8 text-center lg:text-left animate-in fade-in slide-in-from-right-8 duration-1000 delay-200">
                    ...
                    <p className="text-zinc-400 text-lg leading-relaxed max-w-lg mx-auto lg:mx-0">
                        Seu treino na palma da mão, sem complicações. Acompanhe cada série, cada refeição e cada quilo perdido com precisão cirúrgica.
                    </p>

                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 text-left">
                        {features.map((item, idx) => (
                            <li key={idx} className="flex items-center gap-3 group">
                                <div className="w-6 h-6 rounded-lg bg-orange-500/10 flex items-center justify-center border border-orange-500/20 group-hover:border-orange-500/50 transition-colors">
