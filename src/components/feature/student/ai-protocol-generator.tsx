...
function SelectCard({ label, sub, selected, onClick }: { label: string, sub?: string, selected: boolean, onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className={cn(
                "relative p-4 rounded-2xl border text-left transition-all duration-200 active:scale-95",
                selected
...
function NumberCard({ value, label, selected, onClick }: { value: number, label?: string, selected: boolean, onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className={cn(
                "p-2 sm:p-4 rounded-2xl border text-center transition-all duration-200 active:scale-95 w-full min-w-0",
                selected
                    ? "border-orange-500/60 bg-orange-500/10 shadow-orange-500/10 shadow-lg"
                    : "border-zinc-800 bg-zinc-900/50 hover:border-zinc-700"
            )}
...
function ToggleChip({ label, selected, onClick }: { label: string, selected: boolean, onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className={cn(
                "px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all duration-200 active:scale-95",
                selected
...
function TextArea({ placeholder, value, onChange }: { placeholder: string, value: string, onChange: (v: string) => void }) {
    return (
        <textarea
            className="w-full bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-orange-500/50 resize-none h-28 font-medium transition-colors"
        ...
        return (
            <div className="flex flex-col items-center gap-8 py-8 text-center">
                <div className="relative">
                    ...
                    <div className="relative w-24 h-24 bg-orange-500/10 border border-orange-500/30 rounded-[2rem] flex items-center justify-center">
                ...
                {STEPS.map((s, i) => {
                    ...
                    return (
                        <div key={s} className="flex items-center gap-1 flex-1">
                            <div className={cn(
                                "flex items-center justify-center gap-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-300 shrink-0",
                                "px-2 py-1.5",
            ...
            <div className="min-h-[240px] sm:min-h-[280px]">
                ...
                {step === 'workout' && (
                    <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        {workoutSplit === 'other' && (
                            <input
                                className="w-full bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-orange-500/50 font-medium transition-colors"
                    ...
                    <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <div key={label} className="bg-zinc-900/60 border border-zinc-800/50 rounded-2xl p-4">
                                    <Icon className="w-4 h-4 text-orange-500 mb-2" />
                        ...
                        {error && (
                            <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-medium flex items-start gap-3">
                ...
                <div className="flex flex-col gap-3 pt-4 border-t border-zinc-800/50">
                    ...
                    <div className="flex items-center justify-between">
                        <Button
                            ...
                            onClick={prev}
                            disabled={stepIdx === 0}
                            className="h-10 px-5 rounded-xl bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800 font-black uppercase tracking-widest text-[10px] disabled:opacity-30 transition-all"
                        >
                        ...
                        <Button
                            onClick={next}
                            disabled={!canGoNext()}
                            className="h-10 px-5 rounded-xl bg-orange-500 hover:bg-orange-400 text-zinc-950 font-black uppercase tracking-widest text-[10px] disabled:opacity-40 transition-all"
                        >
                            Próximo
