...
export default async function StudentWorkoutsPage() {
    ...
    if (user?.id) {
        ...
        if (allowManualWorkouts) {
            libraryWorkouts = await getTrainerWorkouts()
        }
    }
                                                ...
                                                {allowManualWorkouts && (
                                                    <div className="flex gap-1">
                                                        <Button asChild size="sm" variant="outline" className="h-10 px-3 border-zinc-700 text-zinc-400 hover:text-white">
                                                            <Link href={`/dashboard/student/workouts/${workout.id}`} className="flex items-center gap-1">
                                        ...
                                        <Card key={workout.id} className="bg-zinc-900/50 border-zinc-800 text-zinc-100 hover:border-orange-500/30 transition-all group rounded-3xl overflow-hidden flex flex-col">
                                            <CardHeader className="p-6 pb-4 flex-1">
                                                ...
                                                <CardDescription className="text-zinc-400 text-[12px] leading-relaxed line-clamp-3 mt-2">
                                                    {workout.description || "Sem descrição adicionada pelo treinador."}
                                                </CardDescription>
                                            </CardHeader>
                                            <CardContent>
                                                ...
                                                <div className="flex items-center justify-between text-sm text-zinc-400 mb-6">
                                                    <span className="text-[10px] font-bold uppercase tracking-widest">{workout.exercises?.[0]?.count || 0} Exercícios</span>
