...
export async function setupAutoTrainingForStudent(studentId: string, profileData: any) {
    ...
    try {
        ...
        // 3. Find 4 basic system exercises (or we can just use generic names and hope they exist, 
        // but it's better to fetch system defaults or fallback to text strings if no UUIDs)
        ...
        // Fallback IDs if system exercises aren't seeded properly yet
        let benchPressId = exercises?.find(e => e.name.toLowerCase().includes('supino'))?.id;
        let squatId = exercises?.find(e => e.name.toLowerCase().includes('agachamento'))?.id;
        let pulldownId = exercises?.find(e => e.name.toLowerCase().includes('puxada'))?.id;
        let legpressId = exercises?.find(e => e.name.toLowerCase().includes('leg press'))?.id;

        // If we don't have UUIDs, we have to create temporary ones or skip (we'll just create them on the fly)
        if (!benchPressId) {
            const { data: bench } = await supabase.from('exercises').insert({ name: 'Supino Reto', is_system_default: true, muscle_group: 'Peito', trainer_id: studentId }).select('id').single();
            benchPressId = bench?.id;
        }
        if (!squatId) {
            const { data: squat } = await supabase.from('exercises').insert({ name: 'Agachamento Livre', is_system_default: true, muscle_group: 'Quadríceps', trainer_id: studentId }).select('id').single();
            squatId = squat?.id;
        }
        if (!pulldownId) {
            const { data: pull } = await supabase.from('exercises').insert({ name: 'Puxada Frontal', is_system_default: true, muscle_group: 'Dorsais', trainer_id: studentId }).select('id').single();
            pulldownId = pull?.id;
        }
        if (!legpressId) {
            const { data: leg } = await supabase.from('exercises').insert({ name: 'Leg Press 45', is_system_default: true, muscle_group: 'Quadríceps', trainer_id: studentId }).select('id').single();
            legpressId = leg?.id;
        }
        ...
        const exercisesToInsert = [
            // Upper A
            { workout_id: workoutId, exercise_id: benchPressId, order_index: 1, warmup_sets: 2, feeder_sets: 1, working_sets: 2, reps: '8-10' },
            { workout_id: workoutId, exercise_id: pulldownId, order_index: 2, warmup_sets: 2, feeder_sets: 1, working_sets: 2, reps: '10-12' },
            // Lower A
            { workout_id: workoutId, exercise_id: squatId, order_index: 3, warmup_sets: 2, feeder_sets: 1, working_sets: 2, reps: '8-10' },
            { workout_id: workoutId, exercise_id: legpressId, order_index: 4, warmup_sets: 2, feeder_sets: 1, working_sets: 2, reps: '10-12' },
        ].filter(e => e.exercise_id); // Ensure no nulls
